...
const ai = new BrowserLocalAI(); const aiInfo = getBrowserAIInfo(); const model = LOCAL_MODELS[0];
function render() {
  ...
  document.querySelectorAll<HTMLElement>('[data-load-model]').forEach(b => b.onclick = loadModel);
  document.querySelector<HTMLFormElement>('#chat-form')?.addEventListener('submit', async e => {
    ...
    try { const answer = await ai.generate(text, { maxTokens: 384 }); const updated = getMessages(); updated.push({ role: 'assistant', text: answer || 'The local model returned an empty response.' }); save(updated); }
...
async function loadModel() {
  if (ai.getStatus() === 'loading') return;
  aiMessage = 'Starting local model download…'; aiProgress = 0; render();
  try { await ai.load(model, (p, text) => { aiProgress = Math.max(0, Math.min(100, Math.round(p * 100))); aiMessage = text || `Downloading model: ${aiProgress}%`; render(); }); aiProgress = 100; aiMessage = '✓ Model ready. You can now turn off the internet and chat.'; render(); }
  catch (error) { aiMessage = error instanceof Error ? error.message : 'Model loading failed.'; render(); }
}
function modelCard() {
  const ready = ai.getStatus() === 'ready'; const loading = ai.getStatus() === 'loading';
  return `<div class="card ai-model-card"><div class="model-head"><div><span class="pill">LOCAL AI</span><h3>${model.name}</h3><p>~${model.sizeMb} MB · ${model.contextLength.toLocaleString()} token context</p></div><strong>${ready ? '🟢 Ready' : loading ? `${aiProgress}%` : aiInfo.webgpu ? '⚡ WebGPU' : '⚠️ WebGPU unavailable'}</strong></div>${loading ? `<div class="progress"><span style="width:${aiProgress}%"></span></div>` : ''}<p>${escapeHtml(aiMessage || (ready ? 'Model is running on this device. Internet is not required for inference.' : 'First download the model while online. It will be cached by your browser.'))}</p>${ready ? '<div class="ai-ready">✓ Local inference enabled</div>' : `<button data-load-model class="primary" ${loading || !aiInfo.webgpu ? 'disabled' : ''}>${loading ? `Downloading ${aiProgress}%…` : 'Download & Load Local AI'}</button>`}</div>`;
}
function chat(m: { role: string; text: string }[], used: number) { const ready = ai.getStatus() === 'ready'; return `<section class="hero"><div class="orb">🧠</div><h1>Local intelligence.<br><em>Connected knowledge.</em></h1><p>MindMesh runs the AI model on your device. No cloud inference.</p></section>${modelCard()}<div class="chat-card"><div class="messages">${m.length ? m.map(x => `<div class="msg ${x.role}"><b>${x.role === 'user' ? 'You' : 'MindMesh'}</b><p>${escapeHtml(x.text)}</p></div>`).join('') : '<div class="empty">Load the local model, then ask MindMesh anything.</div>'}</div><form id="chat-form"><input id="prompt" autocomplete="off" placeholder="Ask MindMesh…" ${ready ? '' : 'disabled'}><button ${ready ? '' : 'disabled'}>Send</button></form><div class="chat-meta">${used}/${plan === 'free' ? limits.free.chats : '∞'} chats used · local storage</div><div class="ai-status">Local AI: <strong>${ai.getStatus()}</strong> · ${model.name} · ${aiInfo.webgpu ? 'WebGPU detected' : 'WebGPU unavailable'}</div><button id="clear" class="ghost">Clear conversation</button></div>`; }
...
function settings() { return `<section><h2>Settings</h2><div class="settings">${modelCard()}<label>Offline mode <input type="checkbox" checked></label><label>Privacy-first storage <input type="checkbox" checked></label><label>Notifications <input type="checkbox"></label><div class="card"><h3>Local AI</h3><p>WebGPU: ${aiInfo.webgpu ? 'available' : 'not available'} · Secure context: ${aiInfo.secureContext ? 'yes' : 'no'} · Model: ${model.name}</p><p>WebLLM runs inference locally in the browser. The model is cached locally after its first download.</p></div><div class="card"><h3>Local data</h3><p>Conversations remain in this browser. No automatic cloud upload.</p></div><div class="card"><h3>Future integrations</h3><p>RAG package indexing · Bluetooth · Wi‑Fi Direct · encryption · AI mesh.</p></div></div></section>`; }

...
export interface LocalAIEngine {
  ...
  isSupported(): boolean;
  load(model: AIModelInfo, onProgress?: (progress: number, text?: string) => void): Promise<void>;
...
export class BrowserLocalAI implements LocalAIEngine {
  ...
  async load(model: AIModelInfo, onProgress?: (progress: number, text?: string) => void): Promise<void> {
    ...
    try {
      ...
      this.engine = await webllm.CreateMLCEngine(model.id, {
        initProgressCallback: (report: { progress?: number; text?: string }) => onProgress?.(report.progress ?? 0, report.text),
      });
      this.model = { ...model, installed: true };
      this.status = 'ready';
    } catch (error) {
      this.engine = null;
      this.status = 'error';
      throw error instanceof Error ? error : new Error('Unable to load the local model.');
    }
  }
  ...
  async unload(): Promise<void> {
    try { await this.engine?.unload?.(); } finally {
      this.engine = null;
      this.model = null;
      this.status = 'idle';
    }
